...
public class FileController {
    ...
    private static class MultipartParser {
        ...
        public java.util.List<ParseResult> parseAll() {
            java.util.List<ParseResult> results = new java.util.ArrayList<>();
            try {
                String dataAsString = new String(data);
                String[] parts = dataAsString.split("--" + boundary);
                for (String part : parts) {
                    if (part.trim().isEmpty() || !part.contains("filename=\"") || !part.contains("\r\n\r\n"))
                        continue;
                    String filenameMarker = "filename=\"";
                    int filenameStart = part.indexOf(filenameMarker);
                    if (filenameStart == -1)
                        continue;
                    ...
                    int contentTypeStart = part.indexOf(contentTypeMarker, filenameEnd);
                    String contentType = "application/octet-stream";
                    if (contentTypeStart != -1) {
                        contentTypeStart += contentTypeMarker.length();
                        int contentTypeEnd = part.indexOf("\r\n", contentTypeStart);
                        contentType = part.substring(contentTypeStart, contentTypeEnd);
                    }

                    String headerEndMarker = "\r\n\r\n";
                    int headerEnd = part.indexOf(headerEndMarker);
                    if (headerEnd == -1)
                        continue;
                    ...
                    byte[] fileContent = part.substring(contentStart, contentEnd).getBytes();
    ...
    private class UploadHandler implements HttpHandler {
        ...
        public void handle(HttpExchange exchange) throws IOException {
            ...
            try {
                ...
                java.util.List<MultipartParser.ParseResult> results = parser.parseAll();
                ...
                for (MultipartParser.ParseResult result : results) {
                    ...
                    if (!first)
                        jsonResponse.append(",");
                    jsonResponse.append("{\"port\":").append(port).append(",\"filename\":\"").append(filename)
                            .append("\"}");
                    first = false;
                }
                jsonResponse.append("]");
